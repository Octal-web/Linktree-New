...
export const NotificationMessage = ({ type, message, show, onClose }) => {
    ...
    return (
        ...
        >
            <div className={`w-full max-w-12 h-14 flex items-center justify-center ${bgColor}`}>{icon}</div>

            <div className="flex-grow text-sm px-4 py-2">
                <p>{message}</p>
            </div>

            <button onClick={onClose} className="p-2 text-gray-500 -mt-1 hover:text-gray-700">
                &times;
            </button>

            <div
                ref={progressRef}
                className="absolute bottom-0 left-12 w-full h-[1px] transition-all duration-[5s] ease-linear bg-secondary"
            ></div>
        </div>
    );
};
