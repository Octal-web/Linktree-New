...
export const IndividualItem = ({ individualContent, controller, index, edit }) => {
    ...
    const handleCheckboxChange = () => {
        ...
        put(route('Manager.' + controller + '.visibilidade', { id: individualContent.id }), {
            preserveScroll: true,
            onSuccess: (response) => {
    ...
    return (
        <tr ref={rowRef} className="bg-slate-50">
            ...
            <td
                className="border px-4 py-4 [&_br]:hidden"
                width={cellWidths[1] || 'auto'}
                dangerouslySetInnerHTML={{ __html: individualContent.titulo || individualContent.nome }}
            />
            ...
            <td className="border max-sm:text-center px-2 md:px-4 py-2 w-1/6 sort-ignore" width={`${cellWidths[3] || 'auto'}`}>
                {edit ? (
                    <Link
                        href={route('Manager.' + controller + '.editar', { id: individualContent.id })}
                    ...
                    <Link
                        href={route('Manager.' + controller + '.visualizar', { id: individualContent.id })}
                        className="h-5 w-5 relative mr-4 z-[1] before:content-[''] before:absolute before:-top-[8px] before:-left-[10px] before:w-9 before:h-9 before:bg-slate-200 before:rounded-full before:-mt-[-2px] before:-z-[1] before:transition-all before:transform before:scale-0 hover:before:scale-100"
                    >
                ...
                <button
                    ...
                    onClick={() => openModal(individualContent.id)}
                    disabled={loading}
                >
                ...
                {isModalOpen && (
                    <ConfirmModal
                        ...
                        type="delete"
                        confirm={route('Manager.' + controller + '.excluir', { id: individualContent.id })}
                    />
                )}
            </td>
        </tr>
