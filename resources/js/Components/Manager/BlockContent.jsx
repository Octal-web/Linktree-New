...
export const BlockContent = ({ content }) => {
    ...
    useEffect(() => {
        if (isUpdated) {
            put(route('Manager.' + content.controller + '.ordenar'), {
                preserveScroll: true,
            });
            setIsUpdated(false);
        }
                ...
                {content.addParametros && content.addParametros.length > 0 ? (
                    <div className="flex">
                        {content.addParametros.map((parameter, index) => (
                            <Link
                                key={index}
                                href={route(`Manager.${content.controller}.adicionar`, { tipo: parameter })}
                    ...
                    <Link
                        href={route('Manager.' + content.controller + '.adicionar', { id: content.addId })}
                        className="flex items-center border border-stroke bg-white px-3 py-2 rounded-md transition-all hover:bg-slate-100 ml-2"
                    >
                        <FontAwesomeIcon icon={faPlus} className="text-slate-700 mr-2" />
                    ...
                    <Link
                        href={route('Manager.' + content.controller + '.adicionar')}
                        className="flex items-center border border-stroke bg-white px-3 py-2 rounded-md transition-all hover:bg-slate-100 ml-2"
                    >
                        <FontAwesomeIcon icon={faPlus} className="text-slate-700 mr-2" />
                        {`Adicionar ${content.nome[1]}`}
                ...
                {content.editavel ? (
                    ...
                    >
                        {state.map((conteudo, index) => (
                            <div
                                key={index}
                                className="relative flex flex-col rounded-md border border-stroke p-4 shadow-sm select-none before:content-[''] before:absolute before:top-0 before:left-0 before:bg-secondary before:w-full before:h-1 before:rounded-t-md"
                            >
                                <IndividualContent
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-x-3 sm:gap-x-6 gap-y-4 sm:gap-y-8">
                        {content.conteudos.map((conteudo, index) => (
                            <div
                                key={index}
                                className="relative rounded-md border border-stroke p-4 shadow-sm before:content-[''] before:absolute before:top-0 before:left-0 before:bg-secondary before:w-full before:h-1 before:rounded-t-md"
                            >
                                <IndividualContent
                                    key={index}
                                    individualContent={conteudo}
        ...
        <div className="mb-6 rounded-sm border border-stroke bg-white px-5 py-5 shadow-md">
            ...
            <div className="mt-10 overflow-x-auto no-scrollbar">
                <table className="w-full min-w-[30rem] border-collapse">
                    ...
                    {content.editavel ? (
                        <ReactSortable animation={150} list={state} setList={setState} forceFallback={true} tag="tbody">
                            {state.map((conteudo, index) => (
                                <IndividualItem
                                    key={index}
                                    individualContent={conteudo}
