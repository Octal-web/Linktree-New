...
export const ImageUploadModal = ({ onImageSelect, onClose }) => {
    ...
    const handleUpload = (e) => {
        ...
        post(route('Manager.Arquivo.enviar'), {
            preserveScroll: true,
            onSuccess: (page) => {
                ...
                <div className="flex mb-4 border-b">
                    <button
                        ...
                        className={`px-4 py-2 font-medium ${
                            uploadType === 'file' ? 'border-b-2 border-blue-500 text-blue-600' : 'text-gray-600 hover:text-gray-800'
                        }`}
                    >
                        Upload de Arquivo
                    </button>
                    <button
                        ...
                        className={`px-4 py-2 font-medium ${
                            uploadType === 'url' ? 'border-b-2 border-blue-500 text-blue-600' : 'text-gray-600 hover:text-gray-800'
                        }`}
                    >
                        URL da Imagem
                    </button>
                ...
                {uploadType === 'file' ? (
                    <div>
                        ...
                        >
                            {selectedFile ? (
                                <div>
                                    <img src={imageUrl} alt="Preview" className="max-w-full max-h-40 mx-auto mb-2 rounded" />
                        ...
                        <input name="img" ref={fileInputRef} type="file" accept="image/*" onChange={handleFileSelect} className="hidden" />
                    </div>
                ) : (
                    <div className="mb-4">
                        ...
                        {imageUrl && (
                            <div className="mt-2">
                                <img src={imageUrl} alt="Preview" className="max-w-full max-h-40 rounded" onError={() => setImageUrl('')} />
                            </div>
                        )}
                    </div>
                )}
